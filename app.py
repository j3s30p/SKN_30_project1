...
# ── 상단 헤더 ─────────────────────────────────────────────────────────────────
col_logo, col_nav = st.columns([3, 7])
with col_logo:
    st.markdown('<div class="header-logo">⚡ <span>서울EV</span> 현황시스템</div>', unsafe_allow_html=True)
with col_nav:
    pages = ["현황", "충전소 맵", "FAQ"]
...
if st.session_state.page == "현황":
    ...
    with map_col:
        ...
        for _, row in district_totals.iterrows():
            ...
            fill_color = "#00e5ff" if is_selected else f"rgb({100-int(ratio*100)},{int(ratio*200)},{255})"
            border_color = "#ffffff" if is_selected else "#00e5ff"
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with map_col2:
        ...
        m2 = folium.Map(location=[center_lat, center_lon], zoom_start=zoom, tiles=None)
        folium.TileLayer(
            tiles="https://{s}.basemaps.cartocdn.com/dark_all/{z}/{x}/{y}{r}.png",
        ...
        # 범례
        legend_html = """
        <div style="position:fixed;bottom:30px;right:30px;z-index:9999;background:rgba(17,24,39,0.9);
                    border:1px solid rgba(255,255,255,0.1);border-radius:10px;padding:12px 16px;font-size:12px;color:#e2e8f0;">
        <b style="font-size:13px">충전기 종류</b><br><br>
        <span style="color:#00e5ff">●</span> 급속 &nbsp;
        <span style="color:#10b981">●</span> 완속 &nbsp;
        <span style="color:#f59e0b">●</span> 초급속
        </div>
        """
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    if not faqs:
        ...
        for faq in faqs:
            ...
            with st.expander(f"{'🚗' if faq['category']=='전기차' else '⚡'} {faq['question']}"):
                st.markdown(f"""
                <div style="font-size:14px;line-height:1.8;color:#cbd5e1;padding:8px 0">
                    {faq['answer']}
                </div>
    ...
    # 문의 CTA
    st.markdown("""
    <div style="background:linear-gradient(135deg,rgba(124,58,237,0.2),rgba(0,229,255,0.1));
                border:1px solid rgba(124,58,237,0.4);border-radius:16px;padding:32px;text-align:center;">
        <div style="font-size:22px;font-weight:800;margin-bottom:8px">원하는 답변을 찾지 못하셨나요?</div>
        <div style="color:#94a3b8;margin-bottom:20px">서울시 전기차 상담센터에 문의하세요</div>
        <div style="display:flex;justify-content:center;gap:16px;flex-wrap:wrap">
            <div style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:10px;padding:14px 28px">
                📞 <b>120</b> (다산콜센터)
            </div>
            <div style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:10px;padding:14px 28px">
                🌐 <b>ev.seoul.go.kr</b>
