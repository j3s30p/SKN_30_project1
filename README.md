...
<table>
  ...
  <tr>
    <td><img src="./assets/chiikawa_sq_1.png" width="60"></td>
    <td align="center"><b>박제섭</b></td>
    <td>PM</td>
    <td>기획 / FAQ 데이터 수집·전처리 / 텍스트 분석 / 위치 데이터 가공</td>
  </tr>

  <tr>
    <td><img src="./assets/chiikawa_sq_2.png" width="60"></td>
    <td align="center"><b>박지유</b></td>
    <td>Data Engineering</td>
    <td>데이터 수집 및 전처리 / 충전소 등록 데이터 가공 / DB 모델링 및 아키텍처 설계 / DB 구축 및 운영 관리 / 회의록 관리</td>
  </tr>

  <tr>
    <td><img src="./assets/chiikawa_sq_3.png" width="60"></td>
    <td align="center"><b>박세빈</b></td>
    <td>Data Engineering</td>
    <td>데이터 수집 및 전처리 / 자동차 등록 데이터 가공 / DB 모델링 및 아키텍처 설계 / DB 구축 및 운영 관리</td>
  </tr>

  <tr>
    <td><img src="./assets/chiikawa_sq_4.png" width="60"></td>
    <td align="center"><b>채동현</b></td>
    <td>Frontend / Visualization</td>
    <td>Streamlit 기반 데이터 대시보드 개발 / UI, UX 설계 및 구현</td>
  </tr>

  <tr>
    <td><img src="./assets/chiikawa_sq_5.png" width="60"></td>
    <td align="center"><b>윤태선</b></td>
    <td>Frontend</td>
    <td>웹 서비스 UI 구현 / 데이터 시각화</td>
  </tr>
</table>
<br>
...
### DB 설계 개요
본 프로젝트의 데이터베이스는 서울시 전기차 등록 현황, 충전소 정보, 연료 유형, FAQ 데이터를
효율적으로 관리하고 분석하기 위해 설계했습니다.

핵심 엔티티는 다음과 같습니다.

- `seoul_car_registrations` : 연도별 · 연료별 · 자치구별 자동차 등록 현황
- `seoul_districts` : 서울시 자치구 정보
- `fuel_types` : 연료 종류 및 친환경 여부
- `ev_charging_stations` : 전기차 충전소 위치 및 충전기 수
- `faq` : 전기차 관련 FAQ 데이터

### 테이블 관계
- 한 자치구(`seoul_districts`)에는 여러 등록 현황 데이터(`seoul_car_registrations`)가 연결됩니다.
- 한 연료 유형(`fuel_types`)은 여러 자치구 등록 현황 데이터(`seoul_car_registrations`)에 연결됩니다.
- 한 자치구(`seoul_districts`)에는 여러 충전소(`ev_charging_stations`)가 존재합니다.
- `faq` 테이블은 특정 테이블에 종속되지 않는 독립 엔티티로 설계했습니다.

### 설계 의도
등록 현황 데이터와 충전소 데이터를 자치구 기준으로 연결하여
지역별 전기차 보급 수준과 충전 인프라 현황을 함께 분석할 수 있도록 구성했습니다.
또한 연료 유형 테이블을 분리하여 전기차와 내연기관 차량을 비교 분석할 수 있게 했습니다.
FAQ 데이터는 별도의 독립 테이블로 관리하여 사용자 검색 기능에 활용했습니다.
